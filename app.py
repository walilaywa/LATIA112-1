...
for idx, doc in enumerate(docs):
    ...
    friendly_sentiment = sentiment_mapping.get(overall_sentiment.lower(), "未知情感")

    # 顯示相應的分數（如果有）
    score = doc.confidence_scores.positive if hasattr(doc.confidence_scores, 'positive') else None

    print(f"Document text: {document_text}")
    print(f"Overall sentiment: {friendly_sentiment}")
